...
# Output: /Emulation/storage/ps4-games/<Game Title>.ps4 → /Emulation/roms/ps4/CUSA*
...
# SFO format: magic header, then key/value table
# TITLE is a null-terminated UTF-8 string
extract_sfo_title() {
    local sfo_file="$1"

    # param.sfo is a binary file with a specific format:
    # - Header: magic(4) version(4) key_offset(4) data_offset(4) entry_count(4)
    # - Entry table: key_offset(2) data_fmt(2) data_len(4) data_maxlen(4) data_offset(4)
...
for game_dir in "$PS4_ROMS"/*/; do
    ...
    # Create symlink: <Game Title>.ps4 -> original CUSA folder
    link_path="$PS4_LINKS/${safe_title}.ps4"

    # Remove stale symlink if target changed
    if [ -L "$link_path" ]; then
        existing_target=$(readlink "$link_path")
        if [ "$existing_target" = "$(realpath "$game_dir")" ]; then
            count=$((count + 1))
            continue
        fi
        rm -f "$link_path"
    ...
    ln -sfn "$(realpath "$game_dir")" "$link_path"
